...
pub async fn members(ctx: &Context, gid: GuildId) {
  let cid = ChannelId::from(
    env::var("CHANNELID_ALL")
  ...
  if let Err(why) = cid
    .edit(
      &ctx.http,
      EditChannel::new().name(format!("Mitglieder: {:?}", count)),
    )
    .await
  {
...
pub async fn members_online(ctx: &Context, gid: GuildId) {
  ...
  let count2 = {
    ...
    let count2 = guild.members_with_status(OnlineStatus::Online).count()
      + guild.members_with_status(OnlineStatus::Idle).count()
      + guild
        .members_with_status(OnlineStatus::DoNotDisturb)
        .count();
    count2
  };
  if let Err(why) = cid
    .edit(
      &ctx.http,
      EditChannel::new().name(format!("Mitglieder Online: {:?}", count2)),
    )
    .await
  {
    println!("Error while renaming: {:?}", why);

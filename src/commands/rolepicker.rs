use serenity::all::{
  CommandInteraction, ComponentInteraction, ComponentInteractionDataKind, ResolvedValue, Role,
  RoleId,
};
...
pub async fn create_picker(ctx: &Context, command: &CommandInteraction) {
  match command.data.options().first().unwrap().value {
    ResolvedValue::Integer(1) => {
      role_picker(ctx, command, "Flaming Cliffs 3".to_string(), get_fc()).await
    }
    ResolvedValue::Integer(2) => {
      role_picker(ctx, command, "Hubschrauber".to_string(), get_helis()).await
    }
    ResolvedValue::Integer(3) => {
      role_picker(ctx, command, "Propellerflugzeuge".to_string(), get_props()).await
    }
    ResolvedValue::Integer(4) => {
      role_picker(ctx, command, "Strahlflugzeuge".to_string(), get_jets()).await
    }
    _ => role_picker(ctx, command, "Sonstige Rollen".to_string(), get_other()).await,
  }
}

async fn role_picker(
  ctx: &Context,
...
pub async fn interaction(ctx: &Context, component: &ComponentInteraction) {
  ...
  for roleop in get_all() {
    let role_parsed: Role = {
      ...
      if guild.role_by_name(&roleop.name).is_none() {
        continue;
      }
  ...
  if sel_roles.first().is_some()
    && get_other()
      .iter()
      .any(|ro| &ro.id == sel_roles.first().unwrap())
  {
    let roleid_parsed: RoleId = {
      if guild
        .role_by_name(
          &get_other()
            .iter()
            .find(|ro| &ro.id == sel_roles.first().unwrap())
            .unwrap()
            .name,
        )
        .is_none()
    ...
    if !role_ids_del.is_empty() {
      role_ids_del.dedup();
      if let Err(why) = user
        .remove_roles(ctx, role_ids_del.clone().as_slice())
        .await
      {
    ...
    if roles_cur.iter().any(|p| p.id == role) {
        ...
        .unwrap()
      {
        let add_roleid_parsed: RoleId = {
          if guild.role_by_name(&add_role).is_none() {
            continue;
          }
        ...
        if user.roles.contains(&add_roleid_parsed) {
          ...
          for orole in &roles_cur {
            if orole.id != role && orole.addi_roles.as_ref().unwrap().contains(&add_role) {
              remove = false
            }
          }
          if remove {
...
async fn success(ctx: &Context, component: &ComponentInteraction) {
  component
    ...
    .await
    .expect("Err while acknowledging ComponentInteraction");
  component
    .clone()
    .message
    .edit(ctx, EditMessage::new().content(""))
    .await
    .expect("Err while editing rolepicker");
}
...
pub fn register() -> CreateCommand {
  CreateCommand::new("role")
    ...
    .set_options(vec![CreateCommandOption::new(
      ...
      "typ".to_string(),
      "Welche Rollenzuweisung?".to_string(),
    )
    .required(true)
